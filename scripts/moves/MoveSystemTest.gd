...
static func test_move_system():
	...
	var basic_attack = MoveFactory.create_move_from_resource(
		"res://scripts/moves/resources/offensive/basic_attack.tres"
	)
	var fire_strike = MoveFactory.create_move_from_resource(
		"res://scripts/moves/resources/offensive/fire_strike.tres"
	)
	var defend = MoveFactory.create_move_from_resource(
		"res://scripts/moves/resources/defensive/defend.tres"
	)
	var heal = MoveFactory.create_move_from_resource(
		"res://scripts/moves/resources/defensive/heal.tres"
	)
	var void_pull = MoveFactory.create_move_from_resource(
		"res://scripts/moves/resources/special/void_pull.tres"
	)
	...
	for move in default_moves:
		print(
			(
				"✓ Default move: "
				+ move.get_display_name()
				+ " - "
				+ move.get_description()
			)
		)
	...
	if fire_strike:
		...
		print(
			(
				"Fire Strike - Special Effects: "
				+ str(fire_strike.move_data.special_effects)
			)
		)
		print(
			(
				"Fire Strike - Cooldown: "
				+ str(fire_strike.move_data.cooldown_turns)
			)
		)
	...
	if fire_strike:
		print(
			(
				"Fire Strike initial cooldown: "
				+ str(fire_strike.get_cooldown_remaining())
			)
		)
		fire_strike.current_cooldown = 2
		print(
			(
				"Fire Strike set cooldown to 2: "
				+ str(fire_strike.get_cooldown_remaining())
			)
		)
		...
		print(
			(
				"Fire Strike after second update: "
				+ str(fire_strike.get_cooldown_remaining())
			)
		)
