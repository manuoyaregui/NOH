...
func _apply_special_effect(effect: String, caster: CombatEntity, target: CombatEntity) -> void:
    match effect.to_upper():
        ...
        "ENERGY_DRAIN":
            var drain_amount = min(target.current_energy, 10)
            target.current_energy -= drain_amount
            caster.current_energy += drain_amount
        "SWAP_STATS":
        ...
        "PSYCHIC_LINK":
            ...
            var shared_damage = move_data.damage / 2
